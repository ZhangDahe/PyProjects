...
while inputs:
    ...
    # 开始select 监听, 对input_list 中的服务器端server 进行监听,活动的返回到readable中.
    ...
    for s in readable:
        ...
        if s is server:
            ...
            # 将客户端对象也加入到监听的列表中.同前,当客户端发送消息时 select 将触发readable
    ...
    # Handle outputs
    #处理完第一个for循环之后,肯定就没有客户端请求, 也没有客户端发送消息时, 开始对发送消息列表进行处理, 是否需要发送消息
    # 存储哪个客户端发送过消息
    for s in writable:
        try:
